...
type Events = {
  [K in keyof HTMLElementEventMap]?: (arg: HTMLElementEventMap[K] & { target: any }) => void
}
...
abstract class Component<P extends Props = any> {
  ...
  constructor({ events, ...props }: P) {
    ...
    this.eventBus.on(PHASES.UPDATE, this._componentDidUpdate.bind(this))
    this.eventBus.on(PHASES.UNMOUNT, this._componentWillUnmount.bind(this))
  ...
  public dispatchComponentDidMount() {
    this.eventBus.emit(PHASES.MOUNT)
  ...
  public dispatchComponentWillUmnout() {
    this.eventBus.emit(PHASES.UNMOUNT)
